    ...
    function defCamposHorario(array $dados = [], bool $show = false, int $pos = 0): array
    {
       ...
       $ret['tho_hora_chegada'] = $chegada->crInput();
        ...
        foreach ($dias as $campo => $label) {
            ...
            $dia->dispForm    = 'col-4';
            $dia->ordem       = $pos;
            $dia->selecionado = (isset($dados[$campo])) ? $dados[$campo] : 0;
            $ret[$campo] = $dia->crCheckbox(false);
        }
        
        // ATENDE FERIADO
        $feriado              = new MyCampo('log_transportadora_horarios', 'tho_atende_feriado');
        $feriado->valor       = 1;
        $feriado->label       = 'Feriado';
        $feriado->leitura     = $show;
        $feriado->dispForm    = 'col-4';
        $feriado->ordem       = $pos;
        $feriado->selecionado = (isset($dados['tho_atende_feriado'])) ? $dados['tho_atende_feriado'] : 0;
        
        $ret['tho_atende_feriado'] = $feriado->crCheckbox(false);
